	                  ...
	                  <tbody>	    			
				        <c:forEach items="${postlist}" var="post" varStatus="count">
					        <tr class="tdblue <c:if test="${count.index % 2 == 0}">even</c:if><c:if test="${count.index % 2 != 0}">odd</c:if>">
					        	...
					        	<td class="viewpost"><a href="<c:url value = "${pageurl}"/>?i2=${i1}
					        	<c:if test="${not empty fn:trim(search)}">&search=${search}</c:if>
					        	&idpost=${post.p.id}">
					        	<button type="button" class="btn btn-info"><span class="showbuttext">view ${post.cnt} comments</span> <span class="hidebuttext">view ${post.cnt}</span></button>
					        	</a></td>
					        </tr>
				        </c:forEach>
